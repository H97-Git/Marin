...
public class StatusBarViewModel : ViewModelBase
{
    ...
    public StatusBarViewModel()
    {
        ...
        var assembly = AppDomain.CurrentDomain.GetAssemblies()
            .SingleOrDefault(x => x.GetName().Name == "Marin.Translations");
        if (assembly == null) return;
        var translated =
            TranslationProvider.ReadTranslation(assembly, message.SourceFile, message.Key, new CultureInfo("en-US"));
        if (translated != null)
            SetMessage(new SendMessageToStatusBarCommand(InfoBarSeverity.Informational, translated));
    }
